...
// DatePicker moved to a shared component (web/src/components/ui/DatePicker.tsx) so
// assessments/[id]/edit/page.tsx can reuse it instead of a bare native <input type="date">.
...
function StepCompetencies({
  ...
  const needsCompetencies =
    assessmentType === AssessmentType.COMPETENCY || assessmentType === AssessmentType.FEEDBACK_360;
...
function StepParticipants360({
  ...
  // 'self' is intentionally excluded — the reviewee's own self-assessment is now created
  // automatically when they're added as the assessment's participant (EngineService.
  // addParticipant), so there's no longer a manual "add myself as a rater" step.
                      ...
                      <div className="flex gap-2">
                        ...
                        <div className="relative shrink-0" ref={(el) => { relationshipMenuRefs.current[participant.userId] = el; }}>
                          ...
                          {relationshipMenuOpen[participant.userId] && (
                            <div className="absolute right-0 z-30 mt-1.5 w-full min-w-[160px] overflow-hidden rounded-xl border border-gray-200 bg-white p-1.5 shadow-xl dark:bg-slate-800">
...
function QuestionModeToggle({ value, onChange }: { value: QuestionMode; onChange: (m: QuestionMode) => void }) {
  return (
    <div className="mb-5 inline-flex rounded-xl border border-gray-200 bg-gray-50 p-1">
      {(['competency', 'custom'] as const).map((m) => (
        <button
          key={m}
          onClick={() => onChange(m)}
          className={cn(
            'px-4 py-2 text-sm font-medium rounded-lg transition-all',
            value === m ? 'bg-white shadow-sm text-blue-700' : 'text-gray-500 hover:text-gray-700',
          )}
        >
          {m === 'competency' ? 'Rate standard competencies' : 'Build custom questions'}
        </button>
      ))}
    </div>
  );
...
function Step360Questions({
  ...
  onCompetencyToggle: (id: string) => void;
  questions: FormQuestion[];
  ...
  return (
    <div>
      <QuestionModeToggle value={questionMode} onChange={onModeChange} />
      {questionMode === 'custom' ? (
        <StepQuestions questions={questions} onAdd={onAdd} onUpdate={onUpdate} onRemove={onRemove} />
      ) : (
        <StepCompetencies
          assessmentType={AssessmentType.FEEDBACK_360}
          selected={competencySelected}
          onToggle={onCompetencyToggle}
        />
      )}
    </div>
  );
...
function StepReview({
  ...
  const showCompetencySummary = isCompetency || (is360 && state.questionMode === 'competency');
  const { data: allCompetencies } = useApi<CompetencyDto[]>(showCompetencySummary ? '/items/competencies' : null);
  ...
  return (
    <div>
      ...
      {showCompetencySummary && selectedCompetencyNames.length > 0 && (
        <div className="mb-5 space-y-2">
      ...
      {is360 && state.questionMode === 'custom' && state.questions.length === 0 && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-xs text-yellow-700 mb-5">
...
export default function NewAssessmentPage() {
  ...
  function canProceed(): boolean {
    switch (state.step) {
      ...
      case 3:
        ...
        return true;
      case 4:
        if (is360 && state.questionMode === 'competency') return state.competencyIds.length > 0;
        return true;
      default: return true;
    }
  }
  ...
  return (
        ...
        {state.step === 4 && (
          is360 ? (
            <Step360Questions
              ...
              onModeChange={(m) => update({ questionMode: m })}
              competencySelected={state.competencyIds}
              onCompetencyToggle={(id) => toggleId('competencyIds', id)}
    ...
    {is360 && state.step === 4 && state.questionMode === 'custom' && (
      <AssessmentChatbot
